...
* Uvod[fn:3]
  ...
  _nisu_ ni potrebni[fn:4], kompajlirani kod je _korektan_ i to je dokazivo
  "matematikom koja stoji iza Haskell kompajlera".[fn:5]
...
** Tipovi
   ...
   - Pomaže nam da promislimo i izrazimo svoj program
     ...
     #+BEGIN_QUOTE
     Kao posledicu imamo da kada ne znamo šta treba da radi program ne
     možemo takav program ni da napravimo (kompajliramo). Kolikogod
     prethodna rečenica zvučala paradoksalno svi smo nekada svedočili
     situacijama u kojima je program napisan, i radi, čak iako niko
     nije u stanju da kaže šta treba da radi.
     #+END_QUOTE
...
* Monoidi[fn:6]
  Monoid je veoma jednostavan a moćan koncept koji je obično veoma
  prirodan svakome od nas samo se u programiranju često ne
  koncentrišemo na njega. Sabiranje brojeva čini monoid, Množenje čini
  monoid, Konkatenacija stringova čini Monoid, /future/-i u
  konkurentnom programiranju čine Monoid, događaji (/events/) u
  Functional Reactional Programming čine monoid... Definicija sa
  osloncem na teoriju skupova bi bila:

  #+BEGIN_QUOTE
  Monoid je set sa binarnom operacijom. Operacija mora da bude
  asocijativn i da ima neutralni element (/unit/) u skupu.
  #+END_QUOTE
  ...
  *[Napomena]* Obratite pažnju da komutativnost nije zahtevana za
  monoid iako za sabiranje (množenje) važi *i* komutativnost.
...
** Monoid u teoriji kategorija
   Monoid je kategorija sa jednim objektom sa mnogo morfizama (i
   svakako /id/ morfizmom kao neutralnim elementom).

   [[file:images/monoid_pig.png]]

   Da li od svakog kategoričkog monoida (monoida u teoriji kategorija)
   može da se napravi skup (matematički skup) sa pridruženom binarnom
   operacijom i da to čini monoid (sada u teoriji skupova)?

   Znamo da skup morfizama između dva objekta zovemo *Hom-set*. U
   njemu morfizmi čine elemente. Rezultati monoidalnog produkta
   (binarne operacije iz monoida) između elemenata seta odgovaraju
   ...
   element je /id/ morfizam.

   [[file:images/monoid_set_and_cat.png]]

   Istini za volju (i matematici), morfizmi ne formiraju uvek skup. Za
   one kategorije kod kojih za svaka dva objekta morfizmi čine skup
   kažemo da su /lokalno male/ (/locally small/). Taj skup onda zovemo
   Hom-set.

* Funktori[fn:6]
  Ako posmatramo iz ugla teorije kategorija onda je Funktor takođe
  prost koncept:
  ...
  objekte i morfizme iz $C$ u $D$ na takav način da održava strukturu
  ...
  Očuvati strukturu znači takođe i da sve što je bilo kompozicija u
  ...
  funktora $F$, mora postojati morfizam $Fh$ takav da važi $$Fh = Fg
  \circ Ff$$.

  [[file:images/functor_2.png]]

  Na kraju, bitno je da /id/ morfizmi iz $C$ budu mapirani u /id/
  morfizme u $D$ tako da važi $$Fid_a = id_{F_a}$$.

  [[file:images/funct_ids.png]]

  Obratite pažnju da za funktore *nije neophodno* da $C$ i $D$
  kategorije budu razaličite. Funktori koji rade nad jednom istom
  kategorjom se zovu *endofunktori*.

** Primeri funktora u programiranju
   Primeri u Haskell-u predstavljaju endofunktore jer rade na jednom
   kategorijom (*Hask* - *Type* kategorija sa pridruženim /računanjima
   koja se nikada ne završavaju/ - /bottom/s).
...
*** Maybe funktor
    ...
    Kako bismo ispoštovali zahtev da funktor očuvava strukturu ("radi"
    ...
    [[http://learnyouahaskell.com/][Learn Some Haskell For The Great Good]] funktore objašnjava
    ...
    - Napišite instancu =Functor=-a za listu.
    - Zašto Haskelleri insistiraju na =fmap= pre nego na =map= (iako
      postoji u Haskell-u), odakle im to?
...
** Teorija kategorija i programiranje
   - [[https://github.com/hmemcpy/milewski-ctfp-pdf][Category Theory for Programmers]] (knjiga)
   - [[https://www.youtube.com/watch?v=I8LbkfSSR58&list=PLbgaMIhjbmEnaH_LTkxLI7FMa2HsnawM_][Category Theory predavanja Bartoša Milevskog]]

* Footnotes

[fn:6] Iz /Category Theory for Programmers, Bartosz Milewski/

[fn:5] Dokazivost se komplikuje kada se u obzir uzima lenje
izvršavanje i [[https://wiki.haskell.org/Bottom]["bottom"-i]].

[fn:4] Nečega kao što je refleksija nema. U principu, ako želite da
rezonujete o tipovima u vreme izvršavanja, to samo znači da vaši
...
[fn:2] Teorija koja stoji iza Haksell-a zapravo često ima prilično
prostu alternativu mutiranju i sličnim problemima. Teorija kategorija
bi kategoriju koja takve probleme opisuje zvala [[https://bartoszmilewski.com/2014/12/23/kleisli-categories/][Kleisli]] kategorija.
