...
** Anotacije funkcija
   ...
   Funkcijama u Python-u *>=3* se mogu davati opcione [[https://www.python.org/dev/peps/pep-3107/][anotacije]] za
   parametre i povratne vrednosti. Anotacije su izrazi koji se
   izvršavaju u /compile time/-u. Ti izrazi nemaju nikakvu unapred
   određenu ulogu već njihovo značenje može da dopuni neka biblioteka.

   #+BEGIN_SRC python
     def annot_foo(a: 1 + 2, b: "this " + "is b.") -> len([1, 2, 3]):
         pass
   #+END_SRC
   ...
   #+RESULTS:
   : {'b': 'this is b.', 'return': 3, 'a': 3}

** MyPy
   Python je dinamički tipiziran programski jezik. [[http://mypy-lang.org/][MyPy]] je opcioni
   statički /type checker/ za Python. Ako želite da ispitate tipove u
   Python-u pre nego što se program izvrši to možete da uradite
   koristeći MyPy.

   MyPy se oslanja na anotacije koje su podržane od Python-a 3.

   #+INCLUDE: "py_examples/mypy_example.py" src python :eval no
...
* Parsiranje
  ...
  podsetiti se šta čini *jezik*. Pojam *jezika* uzmimo generalno i
  ...
  [[https://en.wikipedia.org/wiki/Parsing][Parsiranje]] je proces analize jezika (kao teksta) prema formalnim
  ...
  rezultat ima jedno ili više [[https://en.wikipedia.org/wiki/Parse_tree][stabala parsiranja]] koja predstavljaju
...
*** Semantika i pragmatika
    ...
    *Semantika* je značenje jezika.
    ...
    Ako obratite pažnju, videćete da pojam *značenje* u rečenici
    "semantika je značenje jezika" ne znači mnogo. Programski jezik
    /znači nešto programeru/ i /znači nešto mašini/. Ta dva značenja
    se bitno razlikuju jer se misaoni procesi bliski čoveku i oni
    bliski mašini razlikuju. Vrlo grubo rečeno:

    - [[https://en.wikipedia.org/wiki/Operational_semantics][Operaciona semantika]] je značenje pri izvršavanju
    - [[https://en.wikipedia.org/wiki/Denotational_semantics][Denotaciona semantika]] je značenje u apstraktnom modelu koji
      usvaja čovek kao alat za rešavanje problema koji je pred
      njim. Taj apstraktni model se opisuje striktno matematički.
...
** Rešenja
   ...
   Takođe možemo vizualizovati i [[http://www.igordejanovic.net/textX/tutorials/robot/#interpreting-model][mogram]]:
