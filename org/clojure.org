...
** Misliti rekurzivno
   ...
   #+INCLUDE: "clojure-ntp/src/clojure_ntp/core.clj" :src clojure :lines "141-175"
...
** Vežbanje 1
   ...
   3. Napisati funkciju =drop-nth= koja prima dva parametra od kojih je
      ...
      #+INCLUDE: "clojure-ntp/src/clojure_ntp/exercises_1.clj" src clojure :lines "42-86"

      Ili koristeći =keep-indexed= funkciju:

      #+INCLUDE: "clojure-ntp/src/clojure_ntp/exercises_1.clj" src clojure :lines "87-138"

      Ili uz malo skupovnih operacija:

      #+INCLUDE: "clojure-ntp/src/clojure_ntp/exercises_1.clj" src clojure :lines "139-161"
   ...
   4. Napravite =flip= funkciju višeg reda koja invertuje parametre
      ...
      #+INCLUDE: "clojure-ntp/src/clojure_ntp/exercises_1.clj" src clojure :lines "162-187"
   ...
   5. Napisati funkciju =b-tree?= koja proverava da li je neka
      ...
      #+INCLUDE: "clojure-ntp/src/clojure_ntp/exercises_1.clj" src clojure :lines "188-208"
...
*** Ref
    ...
    drugim ne uspe --- _nekoordinisani su_. I zato su tu Refs:
...
** /Homoiconicity/ --- "istoizglednost"
   ...
   *Kod je Kralj, podaci se zamišljaju.*
   ...
   terminima tekstualne sintakse. [[https://clojure.org/reference/reader][Možemo reći]] da je sintaksa Lispa
   data u strukturama podataka Lispa.
   #+END_QUOTE

** Navođenje sintakse --- /syntax quoting/
   Sav Lisp kod je ujedno i struktura podataka, a izvršavanje svodi na
   izračunavanje struktura podataka. Ako pak nekada _ne želimo_ da
   neka *struktura podataka == kod* bude računata treba da je
   /quote/-ujemo.
   ...
   #+INCLUDE: "clojure-ntp/src/clojure_ntp/macros.clj" src clojure :lines "98-115"

   Takođe postoji i način da /unquote/-ujete neku formu i da je pri
   tome "odvijete iz zagrada" --- /unquote-splicing/ =~@=:

   #+INCLUDE: "clojure-ntp/src/clojure_ntp/macros.clj" src clojure :lines "116-119"

   Za još priče o /quoting/-u pogledajte [[https://8thlight.com/blog/colin-jones/2012/05/22/quoting-without-confusion.html][ovaj]] post.

** Reader conditionals (.cljc)

** Vežbanje 2
   1. Koristeći makroe proširiti Clojure tako da se u njemu mogu
      definisati funkcije korišćenjem srpske reči za to "дефиниши-ф"
      (Clojure podržava [[https://en.wikipedia.org/wiki/Unicode][Unicode]], možete slobodno da pišete ćirilicom):

      #+INCLUDE: "clojure-ntp/src/clojure_ntp/exercises_2.clj" src clojure :lines "6-21"

      Vaše je da definišete makro =дефиниши-ф=.

      Rešenje:

      #+INCLUDE: "clojure-ntp/src/clojure_ntp/exercises_2.clj" src clojure :lines "3-5"

   2. Učinite isto i za funkcije =println=, =count= i =apply=
      (ćirilična imena za njih se prepuštaju Vašoj mašti).
